# Guide to creating a DSL

So you want to create a DSL interpreter? Welcome!

The goal of the POSL project is to make it easy to write your
own interpreted language.

## Pick a language

You might already have a language in mind that you want to implement.
Technically speaking, POSL can implement in any Turing complete language,
however, there are certain language features that are easier to implement
than others.

To understand how to create the language you want you first need to understand
the mechanics of how POSL processes a language.

[POSL Engine](./posl_diagram.png)

POSL works by creating a base Context object which has all the initial lexeme 
objects that determines how the language will be parsed and converted into a series of
statements.

Each statement is then evaluated and the result of that evaluation put into the token stream.
