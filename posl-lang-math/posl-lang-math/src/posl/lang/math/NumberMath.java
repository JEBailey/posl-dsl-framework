 ...
 * subclass creates a singleton instance to minimize garbage. All methods must
 * be thread-safe.
 * 
 ...
 * This scheme assumes that by default, groovy literals with decimal points are
 * instantiated as BigDecimal objects rather than binary floating points (Float,
 * Double).
 * <li>Do not force the appearance of exactness on a number that is by
 * definition not guaranteed to be exact. In particular this means that if an
 * operand in a NumberMath operation is a binary floating point number, ensure
 * that the result remains a binary floating point number (i.e. never
 * automatically promote a binary floating point number to a BigDecimal). This
 * has the effect of preserving the expectations of binary floating point users
 * and helps performance.
 * <li>Provide an implementation that is as close as practical to the Java 1.5
 * BigDecimal math model which implements precision based floating point decimal
 * math (ANSI X3.274-1996 and ANSI X3.274-1996/AM 1-2000 (section 7.4).
 * </ol>
 * 
 * @author Steve Goetze
 */
public abstract class NumberMath {
	 ...
	 * exception if the right operand (shift distance) is not an integral type.
	 * For the left operand (shift value) also require an integral type, but do
	 * NOT promote from Integer to Long. This is consistent with Java, and makes
	 * sense for the shift operators.
	 */
	public static Number leftShift(Number left, Number right) {
		if (isFloatingPoint(right) || isBigDecimal(right)) {
			throw new UnsupportedOperationException(
					"Shift distance must be an integral type, but " + right
	...
	public static BigDecimal toBigDecimal(Number n) {
		return (n instanceof BigDecimal ? (BigDecimal) n : new BigDecimal(n
				.toString()));
	}

	public static BigInteger toBigInteger(Number n) {
		return (n instanceof BigInteger ? (BigInteger) n : new BigInteger(n
				.toString()));
	}

	/**
	 * Determine which NumberMath instance to use, given the supplied operands.
	 * This method implements the type promotion rules discussed in the
	 * documentation. Note that by the time this method is called, any Byte,
	 * Character or Short operands will have been promoted to Integer. For
	 * reference, here is the promotion matrix: bD bI D F L I bD bD bD D D bD bD
	 * bI bD bI D D bI bI D D D D D D D F D D D D D D L bD bI D D L L I bD bI D
	 * D L I
	 * 
	 * Note that for division, if either operand isFloatingPoint, the result
	 * will be floating. Otherwise, the result is BigDecimal
	 */
	public static NumberMath getMath(Number left, Number right) {
		if (isNaN(left) || isNaN(right)){
			return NaNMath.INSTANCE;
		}
	...
	protected UnsupportedOperationException createUnsupportedException(
			String operation, Number left) {
		return new UnsupportedOperationException("Cannot use " + operation
				+ " on this number type: " + left.getClass().getName()
				+ " with value: " + left);
	}
}
